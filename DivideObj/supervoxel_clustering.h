    ...
    public:
      ...
      public:
          ...
          void updateGrid( SupervoxelHelper* help)
          {
              ...
              decltype(MaxVoxel_) temppair=std::make_pair(res.first->first,res.first->second);
              MaxVoxel_ = temppair.second > MaxVoxel_.second? temppair : MaxVoxel_;
          }
              ...
              if (!MaxEdgeVoxel_.first&&MaxEdgeVoxel_.second!=-1)
              {
                  for (auto tempvoxel : VoxelNum_) {
                      if (edgevoxel_set_.find(tempvoxel.first->getLabel()) != edgevoxel_set_.end()) {
                          decltype(MaxVoxel_) temppair=std::make_pair(tempvoxel.first,tempvoxel.second);
                          MaxEdgeVoxel_ = temppair.second > MaxEdgeVoxel_.second ? temppair : MaxEdgeVoxel_;
                      }
                  }
