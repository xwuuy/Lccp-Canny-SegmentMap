...
//    boost::unordered_set<uint32_t>  marginvoxel_set_;
    ...
    for (size_t y = 0; y < gridheight; y++)
    {
        ...
        {
            if (left) {//最左
                auto temphelp = grid[y][x].getMaxEdgeSupervoxelHelper(edgevoxel_set_);
                if (temphelp) {
            ...
            if (right) {//最右
                auto temphelp = grid[y][bx].getMaxEdgeSupervoxelHelper(edgevoxel_set_);
                if (temphelp) {
                    marginvoxel_set_.insert(temphelp->getLabel());
                    right = false;
                }
    ...
    for (size_t x = 0; x < gridwidth; x++)
    {
        ...
        for (size_t y = 0, by = gridheight - 1; y < gridheight && by >= 0 && (top || bottom);)
        {
            ...
            if (bottom) {//最下
                auto temphelp = grid[by][x].getMaxEdgeSupervoxelHelper(edgevoxel_set_);
                if (temphelp) {
                    marginvoxel_set_.insert(temphelp->getLabel());
                    bottom = false;
                }
